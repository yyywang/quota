...
from app.libs.error_code import ServerError, ClientTypeError, ContentNotSecurity
...
def send_rescued_notice_to_seeker(touser, id):
    """向求喂养者发送救助后信息提醒"""
    template_id = current_app.config['RESCUED_NOTICE_TO_SEEKER_TEMPLATE_ID']
    page = current_app.config['RESCUED_NOTICE_TO_SEEKER_PAGE'] + "?id=" + str(id)
    miniprogram_state = current_app.config['MINIPROGRAM_STATE']
    data = {
        "phrase1": {
            "value": current_app.config['RESCUED_NOTICE_TO_SEEKER_TITLE']
        },
        "phrase2": {
            "value": current_app.config['RESCUED_NOTICE_TIO_SEEKER_CONTENT']
        }
    }
    send_subscribe_msg(touser=touser, template_id=template_id, data=data, page=page,
                       miniprogram_state=miniprogram_state)


def send_rescued_notice_to_rescuer(touser, id):
    """向我能帮用户发送救助后信息提醒"""
    template_id = current_app.config['RESCUED_NOTICE_TO_RESCUER_TEMPLATE_ID']
    page = current_app.config['RESCUED_NOTICE_TO_RESCUER_PAGE'] + "?id=" + str(id)
    miniprogram_state = current_app.config['MINIPROGRAM_STATE']
    data = {
        "phrase1": {
            "value": current_app.config['RESCUED_NOTICE_TO_RESCUER_TITLE']
        },
        "phrase2": {
            "value": current_app.config['RESCUED_NOTICE_TIO_RESCUER_CONTENT']
        }
    }
    send_subscribe_msg(touser=touser, template_id=template_id, data=data, page=page,
                       miniprogram_state=miniprogram_state)
...
def check_msg_security(content):
    """调用微信小程序内容校验接口校验内容安全
    doc_url: https://developers.weixin.qq.com/miniprogram/dev/
            api-backend/open-api/sec-check/security.msgSecCheck.html
    """
    base_url = current_app.config['WX_CHECK_MSG_SECURITY']
    url = base_url + "?access_token=" + get_access_token()
    rq_data = dict(
        content=content
    )
    response = requests.post(url, data=json.dumps(rq_data)).json()
    if response['errcode'] == current_app.config['WX_MSG_OK']:
        pass
    elif response['errcode'] == current_app.config['WX_MSG_NOT_SECURITY']:
        raise ContentNotSecurity()
    else:
        raise ServerError(msg='wx check msg error')
